    ...
    <div className="min-h-screen bg-zinc-950 text-white flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, index) => (
          ...
          >
            {msg.role === 'ai' && <Bot className="text-emerald-500 mt-1" />}
            <div className={`p-4 rounded-2xl max-w-[80%] ${msg.role === 'user' ? 'bg-emerald-600' : 'bg-zinc-800'}`}>
              {msg.role === 'ai' ? (
            ...
            {msg.role === 'user' && <User className="text-emerald-500 mt-1" />}
          </motion.div>
        ))}
        {isLoading && (
          <div className="flex gap-3">
            <Bot className="text-emerald-500" />
            <div className="p-4 rounded-2xl bg-zinc-800">Typing...</div>
          </div>
        )}
      </div>
