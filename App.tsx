...
import { Bot, Image as ImageIcon, Video, Palette, Send, Loader2, Code, Smartphone } from 'lucide-react';
...
export default function App() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const aiText = data.text || data.error || "No response received from AI.";
          ...
          <div className="flex-1 overflow-y-auto space-y-6 mb-20">
            ...
            {isLoading && (
              <div className="flex gap-4">
                <div className="w-8 h-8 rounded-full bg-emerald-900/30 flex items-center justify-center border border-emerald-800"><Bot size={18} className="text-emerald-500" /></div>
                <div className="p-4 rounded-2xl bg-zinc-900 border border-zinc-800"><Loader2 className="animate-spin" size={18} /></div>
              </div>
            )}
          </div>
